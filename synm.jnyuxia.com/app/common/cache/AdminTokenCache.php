...
class AdminTokenCache extends BaseCache
{
    ...
    {
        parent::__construct();
        $this->prefix = config('project.token_token.prefix');
    ...
    {
        //直接从缓存获取
        $adminInfo = $this->get($this->prefix . $token,null,false);
        if ($adminInfo) {
            return $adminInfo;
        }
    ...
    public function setAdminInfo($token)
    {
        ...
        $adminInfo = [
            ...
            'shop_sn'       => $admin->platform_shop['sn'],
            'sid'           => $adminSession->sid,
        ];
        $this->set($this->prefix . $token, $adminInfo, new \DateTime(Date('Y-m-d H:i:s', $adminSession->expire_time)),false);
