...
class OperationLog
{
    ...
    public function handle($response)
    {
        ...
        //需要登录的接口，无效访问时不记录
        if (!$request->controllerObject->isNotNeedLogin() && empty($request->platformAdminInfo)) {
            return;
        }
        ...
        $systemLog->admin_id = $request->platformAdminInfo['platform_admin_id'] ?? 0;
        $systemLog->admin_name = $request->platformAdminInfo['name'] ?? '';
        $systemLog->action = $notes;
        $systemLog->account = $request->platformAdminInfo['account'] ?? '';
