...
class UpgradeLogic extends BaseLogic
{
    ...
    public static function verify($params)
    {
        ...
        $remoteUrl = self::BASE_URL . "/api/version/verify?domain=".$domain."&product_id=6&type=2&version_id=".$params['id']."&link=".$params['link'];
    ...
    {
        $cacheVersion = (new DuokaiCache())->get('version_lists' . $pageNo);
        if (!empty($cacheVersion)) {
            return $cacheVersion;
        }
        if (empty($pageNo) || empty($pageSize)) {
            $remoteUrl = self::BASE_URL . "/api/version/lists?product_id=6&type=2&page=1";
        } else {
            $remoteUrl = self::BASE_URL . "/api/version/lists?product_id=6&type=2&page_no=$pageNo&page_size=$pageSize&page=1";
        }
        $result = Requests::get($remoteUrl);
        $result = json_decode($result->body, true);
        $result = $result['data'] ?? [];
        (new DuokaiCache())->set('version_lists' . $pageNo, $result, 1800);
        return $result;
    }
    ...
    public static function getPkgLine($params)
    {
        $map = [
            ...
            9 => 'platform_package_link',  //平台管理后台更新包
        ];
