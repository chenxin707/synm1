...
class LoginMiddleware
{
    ...
    public function handle($request, \Closure $next)
    {
        ...
        $platformAdminInfo = (new PlatformAdminTokenCache())->getPlatformAdminInfo($token);
        if (empty($platformAdminInfo) && !$isNotNeedLogin) {
        ...
        if ($platformAdminInfo) {
            //获取临近过期自动续期时长
            $beExpireDuration = Config::get('platform.platform_admin_token.be_expire_duration');
            //token续期
            if (time() > ($platformAdminInfo['expire_time'] - $beExpireDuration)) {
                $result = PlatformAdminTokenService::overtimeToken($token);
        ...
        //给request赋值，用于控制器
        $request->platformAdminInfo = $platformAdminInfo;
        $request->platformAdminId = $platformAdminInfo['platform_admin_id'] ?? 0;
